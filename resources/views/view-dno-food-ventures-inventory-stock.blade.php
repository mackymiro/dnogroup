                        ...
                        <div class="card-body">
                        	<table class="table table-bordered">
                        		<thead>
		            	 		...
		            	 		<tbody>
		            	 			<tr>
		            	 				<td>{{ $viewStockDetail[0]->product_name }}</td>
		            	 				<td>{{ $viewStockDetail[0]->unit_price}}</td>
		            	 				<td>{{ $viewStockDetail[0]->supplier}}</td>
		            	 			</tr>
		            	 		</tbody>
                        	</table>
                        	<table class="table table-bordered">
                        		<thead>
		            	 		...
		            	 		<tbody>
		            	 			<tr>
		            	 				<th>TOTAL IN</th>
		            	 				<th>{{ $viewStockDetail[0]->in}} {{ $viewStockDetail[0]->unit}}</th>
		            	 				<td></td>
		            	 			</tr>
		            	 			<tr>
		            	 				<th>TOTAL OUT</th>
		            	 				<th>{{ $viewStockDetail[0]->out}} {{ $viewStockDetail[0]->unit}}</th>
		            	 				<th></th>
		            	 			</tr>
		            	 			<tr>
		            	 				...
		            	 				<th class="bg-info" style="color:white;">
		            	 					<?php
		            	 						$in = $viewStockDetail[0]->in;
		            	 						$out = $viewStockDetail[0]->out;
		            	 						$minus = $in - $out;
		            	 					?>
		            	 					{{ $minus }} {{ $viewStockDetail[0]->unit}}
		            	 				</th>
		            	 				<th></th>
		            	 			</tr>
                        		...
                        		<thead>
		            	 		...
		            	 		<tbody>
		            	 			...
		            	 			<tr>
		            	 				...
		            	 				<td><?= number_format($getViewStockDetail['amount'], 2)?></td>
		...
		<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
			...
			<div class="form-group">
				<div class="form-row">
					<div class="col-lg-2">
						<label>Date</label>
						<input type="text" id="date<?= $getViewStockDetail['id'] ?>" name="date" disabled class="datepicker form-control" value="{{ $getViewStockDetail['date']}}" />
					</div>
					<div class="col-lg-2">
					...
					<div class="col-lg-1">
						<label>QTY</label>
						<input type="text" id="qty<?= $getViewStockDetail['id'] ?>" name="qty" class="form-control" disabled value="{{ $getViewStockDetail['qty']}}" />
					</div>
					@else
					<div class="col-lg-1">
						<label>QTY</label>
						<input type="text" id="qty<?= $getViewStockDetail['id'] ?>" name="qty" class="form-control"  value="{{ $getViewStockDetail['qty']}}" />
					</div>
					@endif
					@if($getViewStockDetail['description'] != "DELIVERY IN")
					<div class="col-lg-1">
						<label>Unit</label>
						<input type="text" id="unit<?= $getViewStockDetail['id'] ?>" name="unit" disabled class="form-control" value="{{ $getViewStockDetail['unit'] }}" />
					</div>
					@else
					<div class="col-lg-1">
						<label>Unit</label>
						<input type="text" id="unit<?= $getViewStockDetail['id'] ?>" name="unit"  class="form-control" value="{{ $getViewStockDetail['unit'] }}" />
					</div>

					@endif
					<div class="col-lg-4">
  						<label>Status</label>
						<select id="status<?= $getViewStockDetail['id']?>" name="status" class=" form-control"> 
					...
					<div class="col-lg-4">
						...
						<input type="text" id="requestingBranch<?=  $getViewStockDetail['id'] ?>" disabled name="requestingBranch" class="form-control" value="{{ $getViewStockDetail['requesting_branch']}}" />
					</div>
					@else
					<div class="col-lg-4">
						<label>Requesting Branch</label>
						<input type="text" id="requestingBranch<?= $getViewStockDetail['id'] ?>" name="requestingBranch" class="form-control" value="{{ $getViewStockDetail['requesting_branch']}}" />
					</div>
					@endif
					@if($getViewStockDetail['description'] != "DELIVERY IN")
					<div class="col-lg-4">
						<label>Cheque No Issued</label>
						<input type="text" id="chequeNoIssued<?= $getViewStockDetail['id'] ?>" disabled name="chequeNoIssued" class="form-control" value="{{ $getViewStockDetail['cheque_no_issued']}}" />
					</div>
					@else
					<div class="col-lg-4">
						<label>Cheque No Issued</label>
						<input type="text" id="chequeNoIssued<?= $getViewStockDetail['id'] ?>" name="chequeNoIssued" class="form-control" value="{{ $getViewStockDetail['cheque_no_issued']}}" />
					</div>
					@endif
				</div>
			</div>
			<div class="form-group">
				<div class="form-row">
					...
					<div class="col-lg-4">
						<label>Remarks</label>
						<input type="text" id="remarks<?= $getViewStockDetail['id'] ?>" name="remarks" class="form-control" value="{{ $getViewStockDetail['remarks']}}" />
					</div>
			...
			<div class="modal-footer">
				...
				<button type="button" onclick="addRemarks(<?=  $getViewStockDetail['id']?>)" class="btn btn-success btn-lg">Add Remarks</button>
			</div>
			</div>
		</div>
		</div>
...
<script type="text/javascript">
	...
	const addRemarks = (id) =>{
		...
		$.ajax({
                type:"PUT",
                url:'/dno-food-ventures/inventory-stock-update/' +id,
                data:{
                    _method:'put',
                    "_token":"{{ csrf_token() }}",
                    "id":id,
